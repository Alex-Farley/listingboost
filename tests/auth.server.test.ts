...
describe("ListingBoost auth boundary", () => {
  ...
  test("maps legacy identity to a ListingBoost-owned user id", async () => {
    const statements = [];
    ...
    const auth = createListingBoostAuthService(database, async () => new Response(JSON.stringify({ id: "legacy-user-123" })));
