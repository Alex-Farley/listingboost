...
export const propertyBriefSchema = z.object({
  ...
  floorArea: z.object({ value: z.number().positive(), unit: z.enum(["sqft", "sqm"]) }).nullable(),
  price: z.object({ amount: z.number().nonnegative(), currency: z.string().trim().length(3).toUpperCase() }).nullable(),
...
/**
 * Canonical property facts supplied or confirmed by the user.
 * "Verified" does not imply independent verification by ListingBoost.
 */
...
export function normalizePropertyBrief(input: PropertyBriefInput): VerifiedPropertyBrief {
  const features = Array.from(new Set((input.explicitFeatures ?? []).map(normalizeFeature).filter(Boolean)));

  return propertyBriefSchema.parse({
    propertyType: input.propertyType.trim().replace(/\s+/g, " "),
    bedrooms: input.bedrooms ?? null,
    bathrooms: input.bathrooms ?? null,
    floorArea: input.floorArea ? { value: input.floorArea.value, unit: input.floorArea.unit } : null,
    price: input.price ? { amount: input.price.amount, currency: input.price.currency.trim().toUpperCase() } : null,
