...
export type CampaignSummary = { id: string; listingUrl: string; eventType: string; brandName: string; status: CampaignState; assetCount: number; createdAt: string; updatedAt: string; };
export type PersistedCampaignAsset = { id: string; title: string; description: string; generationId: string; generationJobId: string | null; mediaType: "image" | "video"; aspectRatio: string; previewUrl: string | null; rawUrl: string | null; status: string; };
export type PersistedCampaign = { id: string; listingUrl: string; details: string; eventType: string; brandName: string; cta: string; sourceImages: {id:string;type?:string}[]; copy: string; plan: string; status: CampaignState; createdAt: string; updatedAt: string; assets: PersistedCampaignAsset[]; };
export type CreateCampaignInput = { listingUrl: string; details: string; eventType: (typeof events)[number]; brandName: string; cta: string; sourceImages: {id:string;type?:string}[]; };
export type SaveCampaignAssetInput = { campaignId: string; title: string; description: string; generationId: string; mediaType: "image" | "video"; aspectRatio: string; };
export type UpdateCampaignInput = { campaignId: string; status?: CampaignState; copy?: string; plan?: string; };

async function requireUserId(database: NonNullable<ReturnType<typeof bindings>["DB"]>) { const user = await createListingBoostAuthService(database).getCurrentUser(); if (!user) throw new Error("Sign in to manage campaigns."); return user.id; }
function db() { const value = bindings().DB; if (!value) throw new Error("Campaign storage is not available."); return value; }
const createSchema = z.object({ listingUrl: z.string().url().max(2048).refine(v => /^https?:\/\//i.test(v)), details: z.string().min(20).max(12000), eventType: z.enum(events), brandName: z.string().max(200), cta: z.string().min(1).max(200), sourceImages: z.array(sourceImage).min(1).max(6), });
...
export const saveCampaignAssetFn = createServerFn({method:"POST"}).validator(z.object({
  campaignId:z.string().uuid(), title:z.string().min(1).max(100), description:z.string().min(1).max(1000), generationId:z.string().min(1), mediaType:z.enum(["image","video"]), aspectRatio:z.string().regex(/^\d+:\d+$/)
})).handler(async ({data}) => {
  ...
  const asset = await database.prepare("INSERT INTO campaign_assets (id,campaign_id,title,description,generation_id,media_type,aspect_ratio,sort_order,asset_key,generation_status,generation_attempt,generation_error,provider_model,provider_job_id,prompt_version,source_media_ids_json,generation_job_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,COALESCE((SELECT MAX(sort_order)+1 FROM campaign_assets WHERE campaign_id=?),0),?,?,?,?,?,?,?,?,?,?) ON CONFLICT(campaign_id,title) DO UPDATE SET description=excluded.description,generation_id=excluded.generation_id,media_type=excluded.media_type,aspect_ratio=excluded.aspect_ratio,generation_status=excluded.generation_status,generation_attempt=excluded.generation_attempt,generation_error=NULL,provider_model=NULL,provider_job_id=excluded.provider_job_id,generation_job_id=NULL,updated_at=excluded.updated_at RETURNING id").bind(crypto.randomUUID(),data.campaignId,data.title,data.description,data.generationId,data.mediaType,data.aspectRatio,data.campaignId,"legacy", "succeeded",0,null,null,data.generationId,null,"[]",null,now,now).first<{id:string}>();
  if (!asset?.id) throw new Error("Campaign asset could not be persisted.");
  const idempotencyKey = `legacy-fnf:${asset.id}:${data.generationId}`;
  const existingJob = await database.prepare("SELECT id,campaign_asset_id,campaign_id FROM generation_jobs WHERE idempotency_key=?").bind(idempotencyKey).first<{id:string;campaign_asset_id:string;campaign_id:string}>();
  let generationJobId = existingJob?.id;
  if (existingJob && (existingJob.campaign_id !== data.campaignId || existingJob.campaign_asset_id !== asset.id)) throw new Error("Legacy generation idempotency key is already owned by another asset.");
  if (!generationJobId) {
    generationJobId = crypto.randomUUID();
    await database.prepare("INSERT INTO generation_jobs (id,campaign_id,campaign_asset_id,asset_key,state,attempt,idempotency_key,provider_key,provider_job_id,created_at,updated_at,completed_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)").bind(generationJobId,data.campaignId,asset.id,"legacy","succeeded",0,idempotencyKey,"legacy-fnf",data.generationId,now,now,now).run();
  }
  await database.prepare("UPDATE campaign_assets SET generation_job_id=?,generation_status='succeeded',provider_job_id=?,updated_at=? WHERE id=? AND campaign_id=?").bind(generationJobId,data.generationId,now,asset.id,data.campaignId).run();
  await database.prepare("UPDATE campaigns SET updated_at=? WHERE id=? AND auth_user_id=?").bind(now,data.campaignId,userId).run();
  return {ok:true as const, generationJobId};
});

export const updateCampaignRecordFn = createServerFn({method:"POST"}).validator(z.object({ campaignId:z.string().uuid(), status:z.enum(CAMPAIGN_STATES).optional(), copy:z.string().max(12000).optional(), plan:z.string().max(12000).optional() })).handler(async ({data}) => {
  const database=db(), userId=await requireUserId(database); if(!await database.prepare("SELECT id FROM campaigns WHERE id=? AND auth_user_id=?").bind(data.campaignId,userId).first()) throw new Error("Campaign not found.");
  const sets:string[]=[]; const values:unknown[]=[];
  if(data.status!==undefined){ const current=await database.prepare("SELECT status FROM campaigns WHERE id=? AND auth_user_id=?").bind(data.campaignId,userId).first() as {status?:unknown}|null; if(!current || typeof current.status!=="string") throw new Error("Campaign not found."); assertCampaignTransition(current.status,data.status); sets.push("status=?");values.push(data.status); }
  if(data.copy!==undefined){sets.push("copy=?");values.push(data.copy);} if(data.plan!==undefined){sets.push("plan=?");values.push(data.plan);} sets.push("updated_at=?");values.push(new Date().toISOString(),data.campaignId,userId);
  await database.prepare("UPDATE campaigns SET "+sets.join(",")+" WHERE id=? AND auth_user_id=?").bind(...values).run(); return {ok:true as const};
});

export const listCampaignsFn = createServerFn({method:"POST"}).handler(async () => {
  const database=db(), userId=await requireUserId(database); const sql = "SELECT c.id,c.listing_url,c.event_type,c.brand_name,c.status,c.created_at,c.updated_at,COUNT(a.id) asset_count FROM campaigns c LEFT JOIN campaign_assets a ON a.campaign_id=c.id WHERE c.auth_user_id=? AND c.status=\"ready\" GROUP BY c.id ORDER BY c.updated_at DESC LIMIT 50"; const result=await database.prepare(sql).bind(userId).all();
  return (result.results??[]).map(row=>{ const r=row as Record<string,unknown>; return {id:String(r.id),listingUrl:String(r.listing_url),eventType:String(r.event_type),brandName:String(r.brand_name??""),status:normalizeCampaignState(String(r.status)),assetCount:Number(r.asset_count??0),createdAt:String(r.created_at),updatedAt:String(r.updated_at)} satisfies CampaignSummary; });
});

function mediaFromGeneration(generation:unknown,fallback:"image"|"video"){ const g=generation&&typeof generation==="object"?generation as Record<string,unknown>:{}; const result=g.results&&typeof g.results==="object"?g.results as Record<string,unknown>:{}; const rawUrl=typeof result.rawUrl==="string"?result.rawUrl:null; const previewUrl=typeof result.minUrl==="string"?result.minUrl:typeof result.thumbnailUrl==="string"?result.thumbnailUrl:rawUrl; return {previewUrl,rawUrl,status:typeof g.status==="string"?g.status:"unknown",mediaType:fallback}; }
function durableGenerationMedia(job: {state: string; id: string;}, mediaType: "image" | "video") { const state = job.state as GenerationState; return { generationJobId: job.id, previewUrl: null, rawUrl: null, status: campaignAssetStatusFromGenerationState(state), mediaType, }; }
