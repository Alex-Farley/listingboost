...
export type CampaignSummary = {
  ...
  status: string; assetCount: number; createdAt: string; updatedAt: string;
};
...
export type PersistedCampaign = {
  ...
  plan: string; status: string; createdAt: string; updatedAt: string;
...
export type UpdateCampaignInput = {
  campaignId: string; status?: "building" | "ready" | "error";
...
export const listCampaignsFn = createServerFn({method:"POST"}).handler(async () => {
  ...
  return (result.results??[]).map(row=>{
    ...
    return {id:String(r.id),listingUrl:String(r.listing_url),eventType:String(r.event_type),brandName:String(r.brand_name??""),status:String(r.status),assetCount:Number(r.asset_count??0),createdAt:String(r.created_at),updatedAt:String(r.updated_at)} satisfies CampaignSummary;
  });
});
...
export const getCampaignFn = createServerFn({method:"POST"}).validator(z.object({campaignId:z.string().uuid()})).handler(async ({data}) => {
  ...
  return {id:String(row.id),listingUrl:String(row.listing_url),details:String(row.details),eventType:String(row.event_type),brandName:String(row.brand_name??""),cta:String(row.cta),sourceImages,copy:String(row.copy??""),plan:String(row.plan??""),status:String(row.status),createdAt:String(row.created_at),updatedAt:String(row.updated_at),assets} satisfies PersistedCampaign;
});
...
export const duplicateCampaignFn = createServerFn({method:"POST"}).validator(z.object({campaignId:z.string().uuid()})).handler(async ({data}) => {
  ...
  const insert = "INSERT INTO campaigns (id,user_id,listing_url,details,event_type,brand_name,cta,source_images_json,copy,plan,status,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?, 'ready',?,?)";
