...
export const updateCampaignRecordFn = createServerFn({method:"POST"}).validator(z.object({
  ...
  if(data.status!==undefined){
    const current=await database.prepare("SELECT status FROM campaigns WHERE id=? AND auth_user_id=?").bind(data.campaignId,userId).first() as {status?:unknown}|null;
  ...
  await database.prepare("UPDATE campaigns SET "+sets.join(",")+" WHERE id=? AND auth_user_id=?").bind(...values).run();
  return {ok:true as const};
});
