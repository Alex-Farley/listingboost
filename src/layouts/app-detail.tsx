...
const STYLES = ["Property-faithful enhancement"];
const QUALITIES = ["Standard","High","Ultra"];
const LIGHTING = ["Preserve source lighting"];
      ...
      <div className="relative grid min-h-[620px] lg:grid-cols-[0.92fr_1.08fr]">
        <div className="relative z-10 flex flex-col justify-between gap-12 px-6 py-10 sm:px-10 sm:py-12 lg:px-14 lg:py-14">
          <div className="flex max-w-2xl flex-col gap-6">
            ...
            >
              Upload your property details and photography. ListingBoost creates a coordinated marketing pack — with property imagery kept faithful to the source, and any visualisation of potential changes clearly separated and disclosed.
            </Typography>
        ...
        <div className="flex flex-col gap-8">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            ...
            <Typography as="p" variant="body-sm-regular" color="secondary" className="!text-[#68645d] max-w-md text-pretty">
              The campaign stays centred on the property. Each asset has a clear job, while property imagery is subject to a separate fidelity check before it can be treated as publishable.
            </Typography>
          </div>
...
function Hero({libraryItems,libraryPagination,onUpload,openCampaignId,onNewCampaign}:{libraryItems:AssetLibraryItem[];libraryPagination:AssetLibraryPagination;onUpload:(file:File)=>Promise<AssetSelection>;openCampaignId:string|null;onNewCampaign:()=>void}){
 ...
 <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden bg-q-background-secondary px-4 py-5 lg:h-full"><div className="flex min-h-0 flex-1 flex-col gap-5 overflow-y-auto pr-1 lg:pb-28">
  ...
  <Accordion.Root multiple={false} variant="list"><Accordion.Item value="settings"><Accordion.Trigger start={<Icon size="sm" as={IconTune}/>}>Image safety & output settings</Accordion.Trigger><Accordion.Panel><div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><SettingSelect label="Image treatment" icon={IconTune} value={style} onChange={setStyle} options={STYLES}/><SettingSelect label="Quality" icon={IconHighQuality} value={quality} onChange={setQuality} options={QUALITIES}/><SettingSelect label="Source lighting" icon={IconLightMode} value={lighting} onChange={setLighting} options={LIGHTING}/></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-q-300 border border-q-border-subtle bg-q-background-secondary p-4"><Typography as="p" variant="caption-sm-semi-bold" color="primary">ENHANCE · PROPERTY-FAITHFUL</Typography><Typography as="p" variant="caption-sm-regular" color="secondary" className="mt-1">The property must remain unchanged. Independent layout verification is required before an enhanced photograph can be downloaded or treated as publishable.</Typography></div><div className="rounded-q-300 border border-q-border-subtle bg-q-background-secondary p-4 opacity-70"><Typography as="p" variant="caption-sm-semi-bold" color="primary">POTENTIAL VISUALISATION · SEPARATE</Typography><Typography as="p" variant="caption-sm-regular" color="secondary" className="mt-1">Changing the property is a different mode. It must be explicitly disclosed as digitally generated and illustrative; it is not available as a publishable output in this preview.</Typography></div></div></Accordion.Panel></Accordion.Item></Accordion.Root>
  <div className="mt-1 flex flex-col gap-3 rounded-q-400 border border-q-border-subtle bg-q-background-primary p-4"><div className="flex flex-col gap-2"><Typography as="p" variant="caption-sm-regular" color="secondary">Property photography is handled in two distinct ways: Enhance must preserve the property and requires independent verification; Potential Visualisation may change it, but is clearly disclosed as illustrative.</Typography><Typography as="p" variant="caption-sm-regular" color="secondary">A campaign uses five paid generations: four images and one Property Reel. You'll see the estimated credit use before anything is generated.</Typography></div>{error?<div className="flex flex-col gap-2 rounded-q-300 border border-q-border-error bg-q-state-error-bg p-3"><Typography as="p" variant="caption-sm-regular" color="danger" role="alert">{error}</Typography><Button variant="tertiary" size="xs" onClick={()=>void preflightCampaign()}>Review remaining cost & retry</Button></div>:null}</div></div><div className="relative shrink-0 pt-2 lg:pointer-events-none lg:absolute lg:inset-x-4 lg:bottom-5"><Button variant="marketingPrimary" size="lg" className="pointer-events-auto relative w-full" disabled={!canGenerate||busy||preflighting} onClick={()=>void createCampaign()} end={busy||preflighting?<Loader size="xs" color="neutral"/>:<Sparkles width={18} height={18}/>}>{busy?"Creating your campaign":preflighting?"Checking credit use…":photos.length===0?"Add photography to continue":completedAssets.length>0?"Review remaining cost & retry":"Review cost & create campaign"}</Button></div></div>
  <div className="relative flex min-h-[420px] flex-1 flex-col justify-center gap-5 rounded-q-500 bg-q-background-primary p-5"><CampaignProgress progress={progress} busy={busy}/><Typography as="p" variant="caption-sm-regular" color="secondary">Property imagery remains a review step. No generated property image is presented as approved for publication unless the required fidelity checks have passed. Nothing is published automatically.</Typography></div></Card></div></>;
}
...
function CampaignAssetCard({asset,onRefine}:{asset:CampaignAsset;onRefine:()=>void}){
 ...
 const propertyMedia=isVideo||asset.mediaType==="image";
 return <Card surface="solid" className="overflow-hidden rounded-q-500 border border-q-border-subtle">
  <div className="relative grid place-items-center overflow-hidden bg-q-background-secondary" style={{aspectRatio:asset.aspectRatio.replace(":", " / ")}}>{isVideo&&rawUrl?<video src={rawUrl} controls playsInline className="h-full w-full object-cover"/>:!isVideo&&previewUrl?<img src={previewUrl} alt={asset.title} className="h-full w-full object-cover"/>:<div className="p-4"><Typography as="p" variant="caption-sm-regular" color="secondary">Media unavailable</Typography></div>}{propertyMedia&&rawUrl?<div className="absolute left-2 top-2 rounded-q-200 border border-white/30 bg-black/75 px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-white">Preview only · fidelity check required</div>:null}</div>
  <div className="flex flex-col gap-2 p-4"><Typography as="h3" variant="body-md-semi-bold" color="primary">{asset.title}</Typography><Typography as="p" variant="caption-sm-regular" color="secondary">{isVideo?"Property reel preview — not approved for publication.":"Property creative preview — not approved for publication until independent fidelity verification passes."}</Typography>
   <div className="grid grid-cols-2 gap-2"><Button variant="tertiary" size="sm" onClick={onRefine} disabled={propertyMedia}>Refine</Button><Button variant="tertiary" size="sm" disabled={!rawUrl||propertyMedia} onClick={()=>rawUrl&&!propertyMedia&&void downloadMedia(rawUrl,"listingboost-"+asset.title.toLowerCase().replaceAll(" ","-").replaceAll("/","-")+"."+(isVideo?"mp4":"jpg"),generationId,isVideo?"video":"image")}>{propertyMedia?"Awaiting verification":"Download"}</Button></div>
  </div>
 </Card>;
}
