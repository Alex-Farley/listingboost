...
function GalleryTileComponent({ item, rect, top, tier, reducedMotion }: GalleryTileProps) {
  ...
  const actions: CardAction[] = [
    { id: "download", label: "Download", icon: IconDownload, onSelect: () => void downloadMedia(item.kind === "video" ? (item.videoSrc ?? item.src) : item.src, undefined, item.id, item.kind === "video" ? "video" : "image") },
    {
      id: "copy",
      label: "Copy prompt",
