...
import { APP_DETAIL_JOBS, getFnfScopeKey, getSignInUrl } from "@/lib/fnf.browser";
import { useFnfJobClient, useFnfScopeKey, useGenerationRun } from "@higgsfield/fnf-react";
...
function InfoPanel({
  ...
  const handleTurnToVideo = async () => {
    if (!data.id || data.mediaType === "video" || run.status === "submitting") return;
    const signInUrl = getSignInUrl(scopeKey ?? "guest", window.location.pathname + window.location.search + window.location.hash);
    if (signInUrl != null) {
    ...
    await run.start({
      model: "seedance_2_5",
      prompt: {
        instruction: "Animate this real-estate property image into a polished 5-second social-media video. Preserve the exact architecture, room layout, materials and proportions. Use subtle premium camera movement and natural environmental motion only. Do not add, remove or invent property features, text, logos, prices or addresses.",
      },
      media: { image: [{ id: data.id, type: "image_job", url: data.src }] },
      settings: {
        duration: 5,
        resolution: "720p",
        aspectRatio: "16:9",
  ...
  const primary = primaryAction === undefined
    ? (data.id && data.mediaType !== "video" ? { ...DEFAULT_PRIMARY_ACTION, onSelect: handleTurnToVideo } : null)
    : primaryAction;
  const footerActions = (actions ?? DEFAULT_ACTIONS).map((action) =>
    action.id === "download" && action.onSelect == null
      ? { ...action, onSelect: () => void downloadMedia(data.src, undefined, data.id, data.mediaType === "video" ? "video" : "image") }
      : action,
  );
  ...
  return (
    <aside className="flex h-full w-full flex-col gap-2 p-2">
      ...
      {run.error ? <Typography as="p" variant="caption-sm-regular" color="danger" className="px-2" role="alert">{run.error.message}</Typography> : null}
      {run.status === "submitting" ? <Typography as="p" variant="caption-sm-regular" color="secondary" className="px-2">Creating video…</Typography> : null}
